    ...
    for K in [1, 3, 5]:
        ...
        for i in range(test_X.shape[0]):
            ...
            for j in range(train_X.shape[0]):
                distance_vector[j] = myDML.Euclidean_distance(test_X[i], train_X[j])#计算测试集和训练集之间的距离
            labels_of_K_neighbor = train_Y[distance_vector.argsort()[0:K]]#取前k小个进行投票
            predict_label[i] = collections.Counter(labels_of_K_neighbor).most_common(n=1)[0][0]#Counter计算元素的个数 most_common取TopN列表
